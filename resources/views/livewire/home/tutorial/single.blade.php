...
<div id="pos-article-text-card-106821"></div>
        <div>
            ...
            <div class="flex justify-around border-2 h-auto rounded-md">
                @foreach($relatedTutorials as $tutorial)
                    <div
                        class="w-[150px] h-[150px] transition-all duration-500 hover:scale-[1.01] transform origin-center sugg">
                        <img src="{{ asset('storage/'.$tutorial->pic) }}" alt="{{ $tutorial->title }}" loading="lazy"
                             class="w-[100px] h-[100px] rounded-[50%] img-sugg "><br>
                        <h1 class="text-sky-400 hover:text-sky-700 text-[16px] mobile-single-title ">{{$tutorial->title}}</h1>
